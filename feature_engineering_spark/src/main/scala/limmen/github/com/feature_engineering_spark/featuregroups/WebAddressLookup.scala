...
object WebAddressLookup {
  ...
  def computeFeatures(spark: SparkSession, input: String, featuregroupName: String,
    ...
    val numClusters = 5
    val description = "lookup table for id to web_address, used when converting from numeric to categorical representation and vice verse"
    val primaryKey = "id"
    ...
    if (create) {
      ...
      Hops.createFeaturegroup(spark, webAddressesWithIndex, featuregroupName, featurestore, version, description,
      ...
      Hops.insertIntoFeaturegroup(spark, webAddressesWithIndex, featuregroupName, featurestore, version, "overwrite",
