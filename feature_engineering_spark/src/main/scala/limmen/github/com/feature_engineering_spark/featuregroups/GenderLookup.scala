...
object GenderLookup {
  ...
  def computeFeatures(spark: SparkSession, input: String, featuregroupName: String,
    ...
    val numClusters = 5
    val description = "lookup table for id to gender type, used when converting from numeric to categrorical representation and vice verse"
    val primaryKey = "id"
    ...
    if (create) {
      ...
      Hops.createFeaturegroup(spark, genderWithIndex, featuregroupName, featurestore, version, description,
      ...
      Hops.insertIntoFeaturegroup(spark, genderWithIndex, featuregroupName, featurestore, version, "overwrite",
