...
object CustomerTypeLookup {
  ...
  def computeFeatures(spark: SparkSession, input: String, featuregroupName: String,
    ...
    if (keepInMemory) {
      val customerTypeLookupList: Array[Row] = customerTypesWithIndex.collect
      val customerTypeLookupMap: Map[String, Long] = customerTypeLookupList.map((row: Row) => {
        row.getAs[String]("customer_type") -> row.getAs[Long]("id")
      }).toMap
      return customerTypeLookupMap
    }
    ...
    val numClusters = 5
    val description = "lookup table for id to customer type, used when converting from numeric to categrorical representation and vice verse"
    val primaryKey = "id"
    ...
    if (create) {
      ...
      Hops.createFeaturegroup(spark, customerTypesWithIndex, featuregroupName, featurestore, version, description,
      ...
      Hops.insertIntoFeaturegroup(spark, customerTypesWithIndex, featuregroupName, featurestore, version, "overwrite",
