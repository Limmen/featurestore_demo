...
object BrowserActionLookup {
  ...
  def computeFeatures(spark: SparkSession, input: String, featuregroupName: String, version: Int,
    partitions: Int, log: Logger, create: Boolean, jobId: Int, keepInMemory: Boolean = false): Map[String, Long] = {
    ...
    val numClusters = 5
    val description = "lookup table for id to browser_action, used when converting from numeric to categorical representation and vice verse"
    val primaryKey = "id"
    ...
    if (create) {
      ...
      Hops.createFeaturegroup(spark, webActionsWithIndex, featuregroupName, featurestore, version, description,
      ...
      Hops.insertIntoFeaturegroup(spark, webActionsWithIndex, featuregroupName, featurestore, version, "overwrite",
